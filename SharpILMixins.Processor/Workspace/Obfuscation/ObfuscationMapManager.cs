...
namespace SharpILMixins.Processor.Workspace.Obfuscation
{
    public class ObfuscationMapManager
    {
        ...
        public List<ObfuscationMap> LoadedMaps { get; } = new List<ObfuscationMap>();
        ...
        {
            return new ObfuscationMap(LoadedMaps.SelectMany(c => c.Entries).ToImmutableArray());
        }

        public ObfuscationMap PerformNameRemapping()
        {
            return new ObfuscationMap(LoadedMaps.SelectMany(m => PerformNameRemapping(m).Entries).ToImmutableArray());
        }

        public ObfuscationMap PerformNameRemapping(ObfuscationMap map)
        {
            var deObfuscationMapEntries = new List<ObfuscationMapEntry>();
            var module = Workspace.CurrentTargetModule ?? throw new MixinApplyException("Unable to perform name remapping because there is no module being processed currently");

            var typeDefs = module.GetTypes().ToList();

            var typeEntries = map.GetEntriesForType(ObfuscationMapEntryType.Type)
                .ToDictionary(c => c.TargetMember, c => (c.DeObfuscatedName, map.Entries.Where(subEntry => c.ParentMember == subEntry.TargetMember)));
            ...
            foreach (var typeDef in typeDefs)
            {
                ...
                deObfuscationMapEntries.Add(new ObfuscationMapEntry(ObfuscationMapEntryType.Type, typeDef.FullName, oldName));
        ...
        private static void RemapSubEntries(TypeDef typeDef, IEnumerable<ObfuscationMapEntry> subEntries, ICollection<ObfuscationMapEntry> deObfuscationMapEntries,
            UTF8String typeName)
        {
            var memberDefs = typeDef.Fields.Cast<IMemberDef>().Concat(typeDef.Methods).Concat(typeDef.Properties).ToList();
